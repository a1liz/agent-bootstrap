...
2. **导航一致性**：所有 HTML 页面共享同一个 `<nav>`，当前页高亮。页面数 ≤6 时使用平铺链接，>6 时使用下拉分组（`nav-dropdown`），组间按读者关注点划分
3. **共享 CSS**：所有 HTML 页面引用同一个 `style.css`（暗色暖调主题 "Warm Precision"），包含 badge、card、TOC、phase、nav-dropdown 等完整组件样式
4. **README 包含服务启动说明**：项目根目录 README 必须包含启动 HTTP 服务的命令和 SSH 隧道说明
5. **禁止占位符**：完成后 `docs/md/` 中不得残留模板占位文本（如"补充…"、"待定义"、"模块 A"）

### 可选页面

**Deep Interviews 页面**（`deep-interviews.html`）：当项目有深度需求访谈或结构化 ADR 讨论记录时，可创建此页面按自顶向下逻辑整理决策过程。使用 `.di-*` 样式类（sticky TOC 侧边栏 + Q&A 卡片布局）。内容按主题组织而非时间顺序，答案经过润色而非逐字转录。如果没有访谈记录或 DESIGN_DECISIONS.md 已充分覆盖，则不应创建此页面。
